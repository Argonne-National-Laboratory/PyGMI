...
from . import Plot2DDataWidget_Ui


class Plot2DDataWidget(QWidget):
    def __init__(self,parent=None,measdata=[[1,3,2],[3,5,7]],
                 header=["index","prime numbers"],SymbolSize=10,linecolor='y',
                 pointcolor='b',title='Plot Window'):
        ...
        self.update_plot_timer.setSingleShot(True)
    ...
    def change_line_color(self,color=None):
        #yellow = 4294967040
        if color==None:color=QColorDialog.getColor(QColor(4294967040),None,
                                                   'Line color',
    ...
    def change_point_color(self,color=None):
        #blue 4279259391
        if color==None:color=QColorDialog.getColor(QColor(4279259391),None,
                                                   'Point color',
                                                   QColorDialog.ShowAlphaChannel)
        if color.isValid():
    ...
    def check_connection(self,state=1):
        """
        check if the pointer to the Master dataset to display
        (self.measdata in Main.py)
        has changed
        """
    ...
    def autoupdate(self,state=1):
        if state:
            ...
            self.update_plot_timer.start(self.ui.refresh_rate.value()*1000)
        else:
