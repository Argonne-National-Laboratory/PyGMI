...
class Script(threading.Thread):
    ...
    def run(self):
        ...
        SRS=instr.instr_8
        #deltasource.setup_delta_Tlink_inf()
        #Instruments set-up
        ...
        I=f.current1
        SRS.set_amplitude(min([1.0,I*1e3])) #Imax=1mA, corresponds to 1V output on SRS
        ...
        while True: #loop and measure indefinitely, until the main process tells to stop
            ...
            freq,V2,theta=SRS.query_f_R_theta()
            if f.instr_on_14:
                try:
                    with reserved_bus_access:
