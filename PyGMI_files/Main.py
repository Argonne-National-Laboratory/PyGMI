...
from PyQt5.QtCore import QTimer
    ##Main user interface
from . import Graphical_User_Interface_Ui
...
for module in Measurements_programs.__all__:
    importlib.import_module(name='.Measurements_programs.'+module,package='PyGMI_files')
...
class start_GUI(QMainWindow):
    def __init__(self):
        ...
        self.ui = Graphical_User_Interface_Ui.Ui_PyGMI()
        ...
        self.plotwindows=[]
        #initialize a list that will retain a reference to the independent instruments panels windows
        self.panelwindows = []
    ...
    def update_list_of_meas_program(self):
        ...
        importlib.reload(Measurements_programs)
        for module in Measurements_programs.__all__:
            if module in already_imported:
                mod = eval('Measurements_programs.'+module)
                mod = importlib.reload(mod)
                importlib.invalidate_caches()
            else:
                mod = importlib.import_module(name = '.Measurements_programs.'+module,
                                        package = 'PyGMI_files')
            Meas_script = mod.Script
    ...
    def create_new_plotwidget(self):
        newwindow=Plot2DDataWidget(self,
                                   self.measdata,
                                   self.current_header,
                                   SymbolSize=self.mainconf['pointsize'],
                                   linecolor=self.mainconf['linecolor'],
                                   pointcolor=self.mainconf['pointcolor'],
                                   title=self.ui.NewPlotWindowTitle.text())
        # a reference to the window must be kept, otherwise the new window is
        # immediately garbage collected !
        self.plotwindows.append(newwindow)
        newwindow.show()

    def closeAllpanelwindows(self):
        for panel in self.panelwindows:
            panel.close()
        self.panelwindows = []
    ...
    def save_data(self):
        ...
        while not(self.data_queue.empty()):
            # NB:Queues are thread-safe: they won't be accessed by several
            # threads at the same time. The lock mechanism is automatically
            # included within them
            # block=False: means do not block execution until some data is
            # put in the queue
            data, note = self.data_queue.get(block=False)
            # the measuring thread may send different types of
            ...
            if note == 'data':
                ...
                # if note==True, "data" is actually a header for the
                # incoming data
                # NEW 'header'
                ...
                try:
                    ...
                    try:
                        # save data even if the savefile could not be created
                        # (the name of which was provided by the user)
                        self.savefile = open("Unsaved-data-"+\
    ...
    def start_measurements(self):
        # fetch the values from the front panel
        ...
        # self.frontpanel, because the measurement thread has started, so just
        # do it before starting the measurement thread
        # 256 is the buffer size in bytes, 'a' is for 'append', in order to
        # ensure never to erase any file
        self.savefile=open(self.frontpanel_values.savefile_txt_input,'a',256)
        # initiate a thread to run the measurements without freezing the
        # front panel
        self.measurements_thread=meas_thread_class(self,
                                                   ...
                                                   #self.instr_IO.connected_instr) for future upgrade
    ...
    def stop_measurements(self):
        ...
        if self.measurements_thread.isAlive():
            ...
            # (this flag is thread safe: it can't be access by both threads
            # at the same time)
            self.measurements_thread_stop_flag.set()
            # wait for the thread to finish safely (it can take a while if
            # the measurement program has few stopping points set up)
    ...
    def savefile_txt_input_open(self):
        fileName, _ = QFileDialog.getSaveFileName(self,"Savefile",
                                                  directory= "./measurements data")
        # if the user chooses 'Cancel' in the dialog,
        # a unicode empty string is returned
